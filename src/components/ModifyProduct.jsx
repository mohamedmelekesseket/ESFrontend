...
const ModifyProduct = ({id,setModify}) => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const res = await axios.put(`https://142.93.171.166/api/Admin/UpdateProduct/${id}`, formData, {
        headers: {
  ...
  const DeleteProduct = async () => {
    try {
      const res = await axios.delete(`https://142.93.171.166/api/Admin/Delete-Product/${id}`, {
        headers: {
          'Authorization': `Bearer ${user.token}`
        }
      });
              ...
              {Object.entries(existingColorImages).map(([color, urls]) =>
                urls.map((url, idx) => (
                  <div key={`existing-${color}-${idx}`} style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                    ...
                    />
                    <img
                      src={`https://142.93.171.166/uploads/${url}`}
                      alt={`${color} ${idx}`}
                      style={{
                        width: "160px",
                        height: "190px",
