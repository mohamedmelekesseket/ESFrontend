...
const ResetPasword = () => {
  ...
  const ResetEmail = async () => {
    ...
    const find = await axios.post("https://142.93.171.166/api/CheckEmail", { email })
    if (find.status === 204) return toast.error('Email Not exist !!')

    try {
      const res = await axios.post("https://142.93.171.166/api/ResetEmail", { email })
      const { code } = res.data
      if (code) {
        setServerCode(code)
        setReset('Rest2')
  ...
  const NewPassword = async () => {
    ...
    try {
      const res = await axios.post(`https://142.93.171.166/api/NewPassword`, { password, email })
      if (res.status === 200) {
