...
const UserBord = () => {
  ...
  const [Users, setUsers] = useState([]);
    ...
    try {
      ...
      const res = await axios.get("https://142.93.171.166/api/Owner/getUser  ",{
        headers: {
  ...
  const updateUser = async(id) => {
    try {
      ...
      // TODO: Replace hardcoded API URL with environment variable for production
      const res = await axios.put(`https://142.93.171.166/api/Owner/Update-User/${id}` ,{
        role
      },{
        headers: {
          'Authorization': `Bearer ${user.token}`
  ...
  const confirmDelete = async (id, toastId) => {
    try {
      // TODO: Replace hardcoded API URL with environment variable for production
      const res = await axios.delete(`https://142.93.171.166/api/Owner/DeleteUser/${id}`,{
        headers: {
          'Authorization': `Bearer ${user.token}`
          }
      });
